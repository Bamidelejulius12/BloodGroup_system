...
data_labels = ['A+', 'A-', 'AB+', 'AB-', 'B+', 'B-', 'O+', 'O-']

# Define the labels and testing directory
blood_group_info = {
    'A+': 'A Positive (A RhD positive)',
    'A-': 'A Negative (A RhD negative)',
    'AB+': 'AB Positive (AB RhD positive)',
    'AB-': 'AB Negative (AB RhD negative)',
    'B+': 'B Positive (B RhD positive)',
    'B-': 'B Negative (B RhD negative)',
    'O+': 'O Positive (O RhD positive)',
    'O-': 'O Negative (O RhD negative)'
}
...
def predict():
    ...
    if file and file.filename.lower().endswith(('png', 'jpg', 'jpeg', 'bmp')):
        ...
        # Get the medical/biological name
        medical_name = blood_group_info.get(predicted_label, "Unknown")

        return render_template(
            'result.html', 
            pred=predicted_label, 
            prob=f"{probability:.2f}%", 
            botanical_name=medical_name,
